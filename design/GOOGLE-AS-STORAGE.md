...
| **GS-7+** | ✅ **SHARPENED 2026-08-24 — a session BASELINE.** `remoteChanged` was `r.updated > lastSyncAt`: one timestamp against every event, and Google stamps `updated` on everything WE write, so our own writes read as remote edits on the next pass. That is what made a sync take five passes to settle. Now the first pull of a session records `{eventId: updated}`, our own writes are folded back into it as Google confirms them, and `outsideEdits` names the tasks a HUMAN touched. The debug log prints them as *“changed in Google, not here”* — the most useful line in it when something looks wrong |
