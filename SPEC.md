...
**Finish-early (`buffer`) weight** — added 2026-08-11. A deadline was only a hard cap on the search window: nothing could be placed past it, and nothing preferred earlier, so a task due Friday 17:00 could land finishing 16:59. Meanwhile `report.js#buildDeadlineBuffer` already measured `deadline − scheduled end` and flagged anything under 24 h — **the app reported on a quality it never optimised for**. Placement now aims to finish **one fifth of the task's own length** before the deadline: an *overrun allowance* ("if this runs 20% over, you still make it"), which scales with the size of the job rather than with how much notice you happened to get. One fifth of the *runway* was rejected as a restatement of `proximity`. It is a **weight, not a rule**, which is what lets an overburdened week or a two-day deadline override it with **no special-case logic**. Tasks without a deadline score a constant and are provably unaffected. For a chunked project the buffer protects the whole remaining amount, not one sitting. See `design/WEEKLY-PLANNING.md` §4.4.
...
**4.4 Virtual occurrences (OD-12):** materialized at read time, id `taskId@<occurrence key>`, behave as fixed anchors; lived data in `occurrenceData`; editing an occurrence writes exceptions/occurrenceData, never the pattern. One task = one identity = continuous ML history across pattern changes.

**The occurrence key** (amended 2026-08-11) is `YYYY-MM-DD` for the **first** session of a day, `YYYY-MM-DD#2` / `#3` for later ones, and `YYYY-MM-DD#add` for an extra session added by exception. A date used to carry exactly one occurrence, and anything else landing on it was **silently dropped** by the dedupe — which blocked two things at once: **twice-a-day patterns** (the evening dose vanished) and **"one extra session this week" on a day the pattern already fills**, the likeliest day to want one. The first session keeps the **bare** key, so every save written before this is byte-identical and every existing exception still matches. Later sessions are numbered by the window's **declared** time, not its final adjusted one, so moving a session cannot renumber it and carry its lived data to a different one. An extra session gets its own namespace rather than the next free ordinal, so adding or removing a pattern window can't shift it. Exceptions match on the **session** key, which is how "skip just the evening dose" is expressible. `dateOfOccurrence()` strips the suffix for the rare caller that needs the calendar date rather than the identity.
...
**Features:** top-N tag indicators, time-of-day bucket (6), day-of-week, duration bucket, priority, day-fill at completion. Target: overall∈[0,1]; `timingFit≠0` doubles sample weight on time features.
...
> **`placedBy` was removed as a feature 2026-09-08** (model layout v6). Measured
> across `ratedSamples()` it got two of four rows wrong. Ticking a task done
> partway through sends an `endTime` change into `updateTask`, which flips
> `placedBy` on any time change — so an engine-placed task the user never
> touched read 1 because they **finished it early**, the outcome leaking into
> the input. In the other direction `ratedSamples` hard-codes `placedBy:'auto'`
> on every materialised occurrence, so a recurring event the user placed by hand
> read 0. What remained was a mixture of "not recurring" and "finished early".
> It was also inert at placement and double-counted `w.stability` (§6), which
> reads `placedBy` directly. The **field is unchanged** — stability,
> re-optimize and carry-over all still read it; only the feature is gone.
> See `design/ML-HEURISTICS-RECOMMENDATIONS.md` §1.2.
>
