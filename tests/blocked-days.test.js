...
describe('a blocked day suppresses recurring sessions', () => {
  // A recurring session is GENERATED, never placed, so it never passes through
  // computeWindows. Thanksgiving break was blocked, tinted, and still had five
  // classes drawn across it.
  const weeklyClass = (s, dayIdx, hh) => s.addFixed({
    title: 'CHEM1109',
    startTime: D(dayIdx, hh),
    endTime: D(dayIdx, hh + 1),
    recurrence: {
      periods: [{ windows: [{ day: ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'][dayIdx], start: `${String(hh).padStart(2, '0')}:00`, end: `${String(hh + 1).padStart(2, '0')}:00` }], interval: 1 }],
      anchorDate: D(dayIdx, 0),
      exceptions: [],
    },
  });

  it('drops the session from the week the day is blocked', () => {
    const s = seed();
    weeklyClass(s, 3, 9); // every Thursday 09:00
    const before = s.getTasksForWeek(MON).filter((t) => t.title === 'CHEM1109');
    expect(before).toHaveLength(1);

    s.blockDay(D(3, 0));
    expect(s.getTasksForWeek(MON).filter((t) => t.title === 'CHEM1109')).toEqual([]);
  });

  it('suppresses, it does not delete — unblocking brings it back', () => {
    const s = seed();
    const parent = weeklyClass(s, 3, 9);
    s.blockDay(D(3, 0));
    // The PATTERN is untouched: a block is a statement about a day and must not
    // quietly edit a rule that outlives it.
    expect(parent.recurrence.exceptions).toEqual([]);
    s.unblockDay(D(3, 0));
    expect(s.getTasksForWeek(MON).filter((t) => t.title === 'CHEM1109')).toHaveLength(1);
  });

  it('leaves the same pattern alone on days that are not blocked', () => {
    const s = seed();
    weeklyClass(s, 3, 9);
    s.blockDay(D(3, 0));
    // Next week's Thursday is a different date and is not blocked.
    const nextWeek = addDays(MON, 7);
    expect(s.getTasksForWeek(nextWeek).filter((t) => t.title === 'CHEM1109')).toHaveLength(1);
  });

  it('frees the time too — a session that does not happen occupies nothing', () => {
    const s = seed();
    weeklyClass(s, 3, 9);
    s.blockDay(D(3, 0));
    // The occurrence is gone from the occupied set as well as the grid, so it
    // cannot go on acting as an anchor for a day nothing may be placed on.
    const occ = s._occupiedExcluding(null, MON);
    expect(occ.some((iv) => dateKey(iv.start) === dateKey(D(3, 0)))).toBe(false);
  });
});
