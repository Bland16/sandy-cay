// CommitmentsEditor — "8 hours of ENGR project by 3 October", authored.
//
...
// thing to drift.
//
// ⚠️ HOURS, not sessions (PLAN D-1, resolved 2026-08-12). "3 × 1h" is already
// expressible through the sitting bounds, and hours are what a week actually
// spends. The store is minutes because every other duration in the engine is
// minutes; the conversion lives here and nothing downstream knows about it.
//
...
import { fmtDur, MONTHS } from '../format.js';
...
/** The one-line summary a row shows. Read it aloud: "8h by 3 Oct, sittings
 *  1h–3h, at most one a day." */
export function commitmentMeta(c) {
  const period = c.from === c.until ? `on ${fmtDay(c.until)}` : `by ${fmtDay(c.until)}`;
  return `${fmtDur(c.amountMin)} ${period} · sittings ${fmtDur(c.minSitting)}–${fmtDur(c.maxSitting)} · max ${c.maxPerDay}/day`;
}
...
export default function CommitmentsEditor({ sched, mutate }) {
  ...
  const addCommitment = () => {
    const c = mutate((s) => s.addCommitment({
      ...
      // A fortnight from today, which is a period long enough to spread across
      // and short enough to be obviously a placeholder you should edit.
  ...
  if (editing) {
    ...
    return (
      ...
      >
        ...
        {/* HOW MUCH — the amount owed, in hours (D-1). */}
        <Field label="how much">
          <input
            ...
            min="0.25"
            step="0.25"
            value={toHours(c.amountMin)}
            onChange={(e) => { const m = fromHours(e.target.value); if (m) patch(c.id, { amountMin: m }); }}
            aria-label="Commitment hours"
          />
  ...
  return (
    <DrillList
      ...
      blurb="How much a period owes — the week decides when."
