...
// nowhere to hide one, which is exactly why this uses the token flow. It is
// origin-restricted, so it is only usable from the origins registered against
// it. Pre-filled for convenience; override it in the Cabana field to point at
// your own Cloud project.
//
// ⚠️ THIS COMMENT USED TO NAME THE ORIGINS ("localhost:5173 + bland16.github.io")
// AND WAS WRONG — 2026-09-03, `localhost:5173` produced "The scheme, domain,
// and/or port ... may not match an authorized JavaScript origin URI". The
// authoritative list is the Google Cloud Console's Clients page and nowhere
// else; a copy here can only drift, and this one cost a session's debugging.
// If you are running locally and sign-in is refused, add your dev origin there
// (scheme + host + port, no path, no trailing slash) — or use the guest door,
